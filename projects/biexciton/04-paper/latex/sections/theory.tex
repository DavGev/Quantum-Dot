...
We consider a GaAs hard-wall semi-ellipsoidal quantum dot with cylindrical
...
Rydberg,
\begin{equation}
 \rB=\frac{4\pi\epsilon_0\epsilon_r\hbar^2}{\meff e^2},
 \qquad
 E_{\mathrm R}=\frac{\hbar^2}{2\meff\rB^2}.
 \label{eq:effective-units}
\end{equation}
Once expressed in these units, the dimensionless energy problem depends on
the material only through the effective-mass ratio
$\eta=\meff/\mheff$. The numerical GaAs calibration is introduced together
with the radiative-lifetime parameters in \cref{sec:observables}.
...
\end{equation}
For the biexciton we use the four-parameter ansatz employed by
Takagahara~\cite{takagahara1989},
\begin{equation}
...
\end{equation}
where $E_g$ is the band gap and $\EX$ is converted from effective Rydbergs
to electronvolts using \cref{eq:effective-units}. The remaining quantity
$\MX$ is the electron--hole coincidence overlap. At
...
For all numerical results we use the single GaAs parameter set
$\epsilon_r=12.8$, $\meff=0.067m_0$, and $\mheff=0.45m_0$
\cite{vurgaftman2001}, which gives
$\rB\simeq\SI{10.11}{nm}$ and
$E_{\mathrm R}\simeq\SI{5.564}{meV}$. The same $\epsilon_r$ is used for
Coulomb screening and for the dielectric factor in \cref{eq:lifetimes}.
The radiative-lifetime calculation additionally uses
$E_g=\SI{1.519}{eV}$ and $E_P=\SI{22.71}{eV}$, following the optical model
of Ref.~\cite{hayrapetyan2019}.
