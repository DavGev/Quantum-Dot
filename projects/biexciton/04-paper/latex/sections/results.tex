...
\cref{tab:energies} of the Appendix. All energies are reported in the effective Rydberg of
\cref{eq:effective-units}; conversion to dimensional units is used only for
the radiative-lifetime calculation. The values include the single-particle
...
energies. The objective is numerically rough, the pattern search is local, and
different parameter combinations can produce energies separated by less than
the practical quadrature resolution. The parameters characterize the retained
variational minimum but are not claimed to be unique.
...
decay-channel convention gives $\tauXX=\tauX/4$. Propagated error bars are
...
scope. The lifetime calculation follows the optical parameter set and
decay-channel convention of Ref.~\cite{hayrapetyan2019}; it should therefore
be interpreted as the radiative lifetime within that model rather than as a
complete experimental decay time.
